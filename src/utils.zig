    ...
    comptime {
        ...
        if ((errArgsTypes.len-1) != args.len) 
            @compileError(std.fmt.comptimePrint(
                "length mismatch between requirements and position of erroneous types. `{}` vs `{}`", 
                .{args.len, errArgsTypes.len}));
...
pub fn assertIsConforming(
    comptime Validator: types.ValidationStruct, 
    comptime Target: types.ValidationStruct, 
    comptime ValidatorReq: types.TypeStruct, 
    comptime TargetReqs: []const types.TypeStruct) []const types.AssertError {
    ...
    for (TargetReqs) |t| {
        // used to get by ambiguous types. decls that are prefixed by _ get the _s stripped when comparing.
        var validatorReqName = std.mem.trimLeft(u8, ValidatorReq.name, "_");
        // check if name of validator function and target function are the same.
        if (!std.mem.eql(u8, validatorReqName, t.name)) continue;
        isMissing = false;
        // assert that they actually are both functions
        if (t.info != .Fn) {
            errors = errors ++ &[_]types.AssertError{.{
                .desc = std.fmt.comptimePrint(
                    target("`{s}`") ++ normal(" in ") ++ target("`{s}`") ++ normal(" is not a function"), 
                    .{t.name, Target.name})
            }};
        }
        if (ValidatorReq.info != .Fn) {
            errors = errors ++ &[_]types.AssertError{.{
                .desc = std.fmt.comptimePrint(
                    target("`{s}`") ++ normal(" in ") ++ target("`{s}`") ++ normal(" is not a function"), 
                    .{ValidatorReq.name, Validator.name})
            }};
        }
        ...
        if (t.info.Fn.args.len != ValidatorReq.info.Fn.args.len) {
            errors = errors ++ &[_]types.AssertError{.{
                .desc = std.fmt.comptimePrint(
                    normal("Expected parameter length ") ++ validator("{},") ++ normal(" found ") ++ target("{}"), 
                    .{ValidatorReq.info.Fn.args.len, t.info.Fn.args.len, })
            }};
    ...
    if (isMissing) {
        errors = errors ++ &[_]types.AssertError{.{
            .desc = std.fmt.comptimePrint(
                normal("expected function ") ++ validator("`{s}`") ++ normal(" with type ") ++ validator("`{any}`"),
                .{ValidatorReq.name, @Type(ValidatorReq.info)})
        }};
    }
    return errors;
}
